...
-- password_hash must be generated by Node.js using bcrypt.
CREATE TABLE users (
...
CREATE INDEX idx_leave_status ON leave_requests(status); -- TIMESHEETS -- Weekly/daily hours logged by employees. 
...
work_date); 
CREATE INDEX idx_timesheet_status ON timesheets(status); -- LIVE TIME TRACKER -- Allows multiple task/project timers. 
...
CREATE INDEX idx_shifts_date_status ON shifts(shift_date, status); -- PERFORMANCE REVIEW CYCLES 
...
CREATE INDEX idx_reviews_status ON performance_reviews(status); -- PAYROLL -- One payroll record per employee per pay period. 
...
pay_period_start, pay_period_end); 
CREATE INDEX idx_payroll_status ON payroll(payment_status); -- PAYSLIPS 
...
ON UPDATE CASCADE ON DELETE CASCADE 
) ENGINE=InnoDB; -- COMPANY SETTINGS 
...
updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE 
CURRENT_TIMESTAMP
) ENGINE=InnoDB; -- EMPLOYEE SETTINGS 
...
employee_settings(employee_id); -- AUDIT LOG -- Records important changes such as leave approvals. 
...
WHERE r.role_name = 'Admin'; -- HR Manager permissions. 
INSERT INTO role_permissions (role_id, permission_id) 
SELECT r.role_id, p.permission_id 
...
('Unpaid Leave', 'Unpaid time away from work', FALSE, 30); -- Employees based on the existing frontend Employee_info.json. 
INSERT INTO employees 
(employee_number, first_name, last_name, email, phone, position_id, 
salary, employment_status, hire_date) 
VALUES 
('EMP001','Sibongile','Nkosi','sibongile.nkosi@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='Software 
Engineer'),70000,'Active','2015-01-01'), 
('EMP002','Lungile','Moyo','lungile.moyo@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='HR 
Manager'),80000,'Active','2013-01-01'), 
('EMP003','Thabo','Molefe','thabo.molefe@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='Quality 
Analyst'),55000,'Active','2018-01-01'), 
('EMP004','Keshav','Naidoo','keshav.naidoo@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='Sales 
Representative'),60000,'Active','2020-01-01'), 
('EMP005','Zanele','Khumalo','zanele.khumalo@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='Marketing 
Specialist'),58000,'Active','2019-01-01'), 
('EMP006','Sipho','Zulu','sipho.zulu@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='UI/UX 
Designer'),65000,'Active','2016-01-01'), 
('EMP007','Naledi','Moeketsi','naledi.moeketsi@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='DevOps 
Engineer'),72000,'Active','2017-01-01'), 
('EMP008','Farai','Gumbo','farai.gumbo@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='Content 
Strategist'),56000,'Active','2021-01-01'), 
('EMP009','Karabo','Dlamini','karabo.dlamini@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE 
position_title='Accountant'),62000,'Active','2018-01-01'), 
('EMP010','Fatima','Patel','fatima.patel@moderntech.com',NULL, 
(SELECT position_id FROM positions WHERE position_title='Customer Support 
Lead'),58000,'Active','2016-01-01'); 
...
FROM employees;
-- Login accounts. 
INSERT INTO users (employee_id, role_id, email, password_hash) 
SELECT e.employee_id, 
CASE 
WHEN e.email = 'lungile.moyo@moderntech.com' THEN (SELECT role_id 
...
FROM employees e; -- Attendance based on the supplied frontend data. 
...
INSERT INTO attendance (employee_id, attendance_date, status) 
SELECT employee_id, '2025-07-29', 'Present' FROM employees WHERE 
employee_number='EMP003'; -- Give the remaining employees sample attendance for the same five days. 
INSERT INTO attendance (employee_id, attendance_date, status) 
...
WHERE e.employee_id >= 4; -- Leave requests 
INSERT INTO leave_requests 
(employee_id, leave_type_id, start_date, end_date, reason, status) 
SELECT e.employee_id, 
(SELECT leave_type_id FROM leave_types WHERE leave_type_name='Sick 
Leave'), 
'2025-07-22','2025-07-22','Sick Leave','Approved' 
FROM employees e WHERE e.employee_number='EMP001'; 
INSERT INTO leave_requests 
(employee_id, leave_type_id, start_date, end_date, reason, status) 
SELECT e.employee_id,
(SELECT leave_type_id FROM leave_types WHERE 
leave_type_name='Personal Leave'), 
'2024-12-01','2024-12-01','Personal','Pending' 
FROM employees e WHERE e.employee_number='EMP001'; 
INSERT INTO leave_requests 
(employee_id, leave_type_id, start_date, end_date, reason, status) 
SELECT e.employee_id, 
(SELECT leave_type_id FROM leave_types WHERE 
leave_type_name='Family Responsibility'), 
'2025-07-15','2025-07-15','Family Responsibility','Denied' 
FROM employees e WHERE e.employee_number='EMP002'; 
INSERT INTO leave_requests 
(employee_id, leave_type_id, start_date, end_date, reason, status) 
SELECT e.employee_id, 
(SELECT leave_type_id FROM leave_types WHERE 
leave_type_name='Annual Leave'), 
'2024-12-02','2024-12-02','Vacation','Approved' 
FROM employees e WHERE e.employee_number='EMP002'; -- Timesheet sample data. 
...
FROM employees WHERE employee_number IN ('EMP001','EMP002','EMP003'); -- Shift sample data. 
INSERT INTO shifts (employee_id, shift_date, start_time, end_time, 
location) 
SELECT employee_id, '2025-07-30', '08:00:00', '17:00:00', 'Main Office' 
FROM employees WHERE employee_number IN ('EMP001','EMP002','EMP003'); -- Review cycle and sample reviews. 
...
((SELECT employee_id FROM employees WHERE 
employee_number='EMP010'),'2025-07-01','2025-07-31',58000,162,0,0,250,0,58000,57750,'Paid'); -- Payslips for the sample payroll. 
...
SELECT employee_id, 'theme', 'light' FROM employees; -- TRIGGERS 
DELIMITER $$ 
...
END$$ 
DELIMITER ; -- OPTIMIZED VIEWS / COMMON QUERIES 
...
-- EXAMPLE QUERIES FOR THE NODE.JS/EXPRESS BACKEND -- GET all employees -- SELECT * FROM vw_employee_directory ORDER BY last_name, first_name; -- Search employees -- SELECT * FROM vw_employee_directory -- WHERE employee_name LIKE CONCAT('%', ?, '%') --    
OR department_name LIKE CONCAT('%', ?, '%') -- ORDER BY employee_name; -- Get one employee -- SELECT * FROM vw_employee_directory WHERE employee_id = ?; -- Get attendance for one employee -- SELECT * FROM attendance -- WHERE employee_id = ? -- ORDER BY attendance_date DESC; -- Get pending leave requests -- SELECT lr.*, CONCAT(e.first_name, ' ', e.last_name) AS employee_name, --        
lt.leave_type_name -- FROM leave_requests lr -- JOIN employees e ON e.employee_id = lr.employee_id -- JOIN leave_types lt ON lt.leave_type_id = lr.leave_type_id -- WHERE lr.status = 'Pending' -- ORDER BY lr.created_at DESC; -- Get payroll for one employee -- SELECT * FROM vw_payroll_summary -- WHERE employee_number = ? -- ORDER BY pay_period_start DESC; -- QUICK TESTS 
